...
class BerandaController extends Controller
{
    ...
    {
        $suratoffering = Surat::find(3); // Pastikan ID sesuai dengan data yang ada
        $dataDetailoffering = array();

        // Ambil data dari model Isi_Surat berdasarkan ID surat dan nama_input
        $fields = [
            'Kepada', 'Posisi', 'Bertugas', 'Tanggal', 'Selama', 'Tertulis', 
            'Terdiri atas', 'Terdiri atas :', 'Melalui', 'Hakcuti', 'Benefit', 'Benefitx'
        ];

        foreach ($fields as $field) {
            $dataDetailoffering[$field] = Isi_Surat::where('id_surat', $suratoffering->id)
                ->where('nama_input', $field)
                ->first()
                ->isi_input ?? ''; // Default ke string kosong jika tidak ditemukan
        }

        // Kirimkan variabel ke tampilan
        return view('layouts.offering', compact('suratoffering', 'dataDetailoffering'));
    }
